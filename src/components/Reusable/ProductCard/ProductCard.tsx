      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-2">
        ...
        <div className="p-4">
            ...
            <div className="flex items-center gap-1">
              {product.colors.slice(0, 3).map((color:any, index:number) => (
                <div
                  ...
                  style={{ 
                    backgroundColor: color.colorName.toLowerCase() === 'orange' ? '#FFA500' : 
                                   color.colorName.toLowerCase() === 'blue' ? '#3B82F6' :
                                   color.colorName.toLowerCase() === 'black' ? '#000000' :
                                   color.colorName.toLowerCase() === 'white' ? '#FFFFFF' :
                                   color.colorName.toLowerCase() === 'red' ? '#EF4444' : '#6B7280'
                  }}
          ...
          <div className="mt-2 flex items-center gap-1">
            ...
            {product.colors[0]?.sizes.slice(0, 3).map((size:any, index:number) => (
              <span key={index} className="text-xs bg-gray-100 px-2 py-1 rounded">
                {size.size}
              </span>
            ))}
            {product.colors[0]?.sizes.length > 3 && (
              <span className="text-xs text-gray-500">+{product.colors[0].sizes.length - 3}</span>
            )}
          </div>
        </div>
      </div>
